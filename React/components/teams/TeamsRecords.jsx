...
function TeamsRecords({
  ...
  function checkUserRole(userRole) {
    if (userRole.includes("OrgAdmin")) {
      return (
        <div className="team-buttonsIcons col">
          <button onClick={onEditClicked} className="btn">
            <Edit className="team-edit-icon" />
          </button>
          <button onClick={onDeleteClicked} className="btn">
            <Trash2 className="team-edit-icon" />
          </button>
        </div>
      );
    }
  ...
  return (
    <div className="card text-dark bg-light-subtle border team-card-shadow team-card-transform">
      <div
        className={`card-body ${isSelected && "team-selected"}`}
